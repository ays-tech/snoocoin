...
const getLevel = (points) => {
  if (points >= 900) return { name: "Satoshi", icon: "🔱", max: 900 };
  if (points >= 800) return { name: "Whale", icon: "🐋", max: 800 };
  if (points >= 700) return { name: "Validator", icon: "🔗", max: 700 };
  if (points >= 600) return { name: "Miner", icon: "⛏️", max: 600 };
  if (points >= 500) return { name: "Trader", icon: "📈", max: 500 };
  if (points >= 400) return { name: "Hodler", icon: "💎", max: 400 };
  if (points >= 300) return { name: "Enthusiast", icon: "🚀", max: 300 };
  if (points >= 200) return { name: "Airdropper", icon: "✈️", max: 200 };
  if (points >= 100) return { name: "Explorer", icon: "🌍", max: 100 };
  return { name: "Newbie", icon: "🌱", max: 0 };
};

export default function TapToEarnTab({ user }) {
  const tapQuantity = 2;
  // const readingCoin = 1000;
    ...
    <div className='p-5 h-[80%]'>
      ...
      <div className='flex justify-between items-center rounded-lg bg-[#002247] py-3 px-4'>
        ...
        <div className='flex flex-col items-center'>
          ...
          <div className='text-yellow-300 font-bold text-xl'>{userLevel.max + 100}</div>
        </div>
      </div>
