...
const getLevel = (points) => {
  if (points >= 100000) return { name: "Satoshi", icon: "🔱", max: Infinity };
  if (points >= 90000) return { name: "Whale", icon: "🐋", max: 100000 };
  if (points >= 80000) return { name: "Validator", icon: "🔗", max: 90000 };
  if (points >= 70000) return { name: "Miner", icon: "⛏️", max: 80000 };
  if (points >= 60000) return { name: "Trader", icon: "📈", max: 70000 };
  if (points >= 50000) return { name: "Hodler", icon: "💎", max: 60000 };
  if (points >= 40000) return { name: "Enthusiast", icon: "🚀", max: 50000 };
  if (points >= 30000) return { name: "Airdropper", icon: "✈️", max: 40000 };
  if (points >= 20000) return { name: "Explorer", icon: "🌍", max: 30000 };
  return { name: "Newbie", icon: "🌱", max: 20000 };
};

export default function TapToEarnTab({ user }) {
  const tapQuantity = 2;
  const { data, isSuccess, isFetched } = useGetPoints(user?.id);
  const [points, setPoints] = useState(
    parseInt(localStorage.getItem("points") || 0)
  );
  const { data: storedPoints, mutate, error } = useStorePoints();
  ...
  useEffect(() => {
    if (data > localStorage.getItem("points")) {
      const point = parseInt(data);
      isSuccess && setPoints(point);
    }
  }, [data]);
  ...
  const storePoints = () => {
    ...
    mutate({ id: user?.id, point: tapQuantity });
  };

  console.log(data, storedPoints, error);
